...
import { ArrowLeft, Download, RefreshCw, Copy, ClipboardEdit } from "lucide-react";
import PasteAnswerModal from "./PasteAnswerModal";
...
export default function QRCodeBlock({ onBack,onConnected }: Props) {
  const [offer,setOffer] = useState<string|null>(null);
  const [sec,setSec]     = useState(QR_TTL);
  const [showPasteAns,setShowPasteAns]=useState(false);
    ...
    <div style={{textAlign:"center",marginTop:32}}>
      ...
      <div style={{marginTop:16,display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
        ...
        {offer && <>
          ...
          <button className="main-btn" onClick={()=>setShowPasteAns(true)}>
            <ClipboardEdit size={16}/> Paste answer
          </button>
        </>}
      </div>

      {offer && <textarea readOnly value={offer}
                          style={{marginTop:16,width:330,height:120,fontSize:12,resize:"none"}}/>}

      {showPasteAns &&
        <PasteAnswerModal onConnected={()=>{setShowPasteAns(false);onConnected();}}
                          onClose={()=>setShowPasteAns(false)}/> }
    </div>
  );
}
